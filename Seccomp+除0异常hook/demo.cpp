...
void sigfpe_handler(int sig, siginfo_t *info, void *ucontext) {
    ucontext_t *uc = (ucontext_t *)ucontext;//获取触发异常之前的上下文
    ...
    // 计算 a 的地址：rbp - 0xC
    int *a_ptr = (int *)(rbp - 0x8); // 这个需要首先反汇编看一下输入到底存在栈的哪个位置
    /*
        类似于
        call    _Z14safe_scanf_intPi ; safe_scanf_int(int *)
        mov     eax, [rbp+var_8]
        你这里的a_ptr就是rbp-0x8
    */
